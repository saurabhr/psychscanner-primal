...
def test_build_ledger_finds_bundled_environment():
    ledger = build_ledger()
    assert "psychscanner_nback" in ledger
    assert ledger["psychscanner_nback"]["path"] == "environments/psychscanner_nback"


def test_no_duplicate_against_its_own_ledger_entry():
    ledger = build_ledger()
    warnings = find_duplicates(ENVIRONMENTS_DIR / "psychscanner_nback", ledger)
    assert warnings == []


def test_flags_name_collision(tmp_path):
    ledger = build_ledger()
    clash = tmp_path / "psychscanner_nback"
    clash.mkdir()
...
def test_flags_content_duplicate(tmp_path):
    ledger = build_ledger()
    copy = tmp_path / "psychscanner_nback_v2"
    copy.mkdir()
    src = ENVIRONMENTS_DIR / "psychscanner_nback" / "nback_demo.json"
